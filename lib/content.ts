    ...
    for (const file of files) {
      ...
      const frontmatterObj = frontmatterRaw.split('\n').reduce((acc, line) => {
        ...
        if (parsedVal.startsWith('[') && parsedVal.endsWith(']')) {
          parsedVal = parsedVal.slice(1, -1).split(',').map(s => s.trim().replace(/['"]/g, ''));
        }
      ...
      const parsed = ProjectFrontmatterSchema.safeParse({ ...frontmatterObj, category });
      if (!parsed.success) {
        console.warn(`Invalid frontmatter in ${filePath}:`, parsed.error.errors);
        continue;
      }
...
export async function getProject(category: string, slug: string) {
  const filePath = path.join(process.cwd(), `content/projects/${category}/${slug}.mdx`);
  const rawContent = fs.readFileSync(filePath, 'utf-8');
  ...
  const validated = ProjectFrontmatterSchema.parse({ ...frontmatter, category });
