import FooterGlow from "../FooterGlow"; // Ensure the path correctly matches your folder structure!
...
export default function Hero({ projectCount }: HeroProps) {
  return (
    /* Added "w-full" and ensured "relative overflow-hidden" to cleanly trap the glowing waves background */
    <section className="relative w-full overflow-hidden pt-32 pb-20 md:pt-44 md:pb-28 px-6 md:px-10">
      
      {/* Background Layer: Positions the rolling waves behind the copy text */}
      <div className="absolute inset-0 pointer-events-none z-0 h-full w-full">
        <FooterGlow />
      </div>

      {/* Main Content: Max-width container remains centered using mx-auto */}
