const socials = [
  ...
  { icon: "fa-x-twitter", label: "X", href: "https://x.com/beyond2021" },
  {
    icon: "fa-linkedin-in",
    label: "LinkedIn",
    href: "https://linkedin.com/in/ioskeevinmitcell",
  },
  {
    icon: "fa-dribbble",
    label: "Dribbble",
    href: "https://dribbble.com/beyond2021",
  },
];
  ...
  return (
    <footer className="relative mt-16 md:mt-24 overflow-hidden">
      ...
      <div className="relative z-10 border-t border-white/5">
        {/* Mobile: 2 columns | Tablet: 3 columns | Desktop: 5 columns custom layout */}
        <div className="px-6 md:px-10 py-12 md:py-14 max-w-7xl mx-auto grid gap-10 grid-cols-2 sm:grid-cols-3 md:grid-cols-[1.4fr_repeat(4,1fr)]">
          {/* Brand block: Spans full width on mobile/tablet to look clean */}
            ...
            <div className="mt-5 flex items-center gap-4">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="text-white/30 hover:text-white p-1 transition-colors touch-manipulation"
                >
                  <i
                    className={`fa-brands ${s.icon} text-lg md:text-base`}
                    aria-hidden="true"
                  />
                </a>
              ))}
              ...
              <ul className="space-y-3 md:space-y-2.5">
                {col.links.map((l) => (
                  <li key={l.label + l.href}>
                    <a
                      href={l.href}
                      className="text-sm text-white/30 hover:text-white/70 block py-1 md:py-0 transition-colors touch-manipulation"
                    >
                      {l.label}
                    </a>
                  </li>
